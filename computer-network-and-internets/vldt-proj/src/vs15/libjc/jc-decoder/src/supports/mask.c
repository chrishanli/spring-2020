...
{
	Int32 mask_type = 0;
	Int32 min_penalty_score = 10000;

	//allocate memory for masked code
    Int32* masked = (Int32 *)malloc(cp->code_size.x * cp->code_size.y * sizeof(Int32));
    if(masked == NULL)
    {
        J_REPORT_ERROR(("%s","Memory allocation for masked code failed"))
        return -1;
    }
	memset(masked, -1, cp->code_size.x * cp->code_size.y * sizeof(Int32)); //set all bytes in masked as 0xFF
