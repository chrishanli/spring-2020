...
// 新建客户端或服务器端监控器
// 本地提供一个检视器TCP端口
func NewMonitor(isClient bool, monPort string, errPort string, isZipNeeded bool, isEncNeeded bool) (ret int32) {
	// 新建一个监控器
	var mon Monitor
	// 各种连接
	...
	for {
		...
		case TUN_CONNECT: // 0
			...
			mon.ClientTask.isZipNeeded = isZipNeeded
			mon.ClientTask.isEncNeeded = isEncNeeded
			break
		case TUN_SEEK: //
			// 查询发送进度，就返回进度
