...
func (d *ConcDesc)ReadDesc () (*ConcDesc, bool) {
	concPath, _ := filepath.Split(os.Args[0])
	if concPath == "" { concPath = ".\\" }
	confPath := concPath+"conf"+string(os.PathSeparator)
	var yamlFile []byte
	var err error
	var isDefault = true
	switch GlobalConfig.AppLang {
	case "zh-CN":
		yamlFile, err = ioutil.ReadFile(confPath+"conc_desc_zh-CN.yaml")
		if err != nil {
			break
		}
	...
	// Default is English
	if isDefault {
		yamlFile, err = ioutil.ReadFile(confPath+"conc_desc_en-UK.yaml")
		if err != nil {
			HdrErr()
			fmt.Println("Unable to reach language document (in", confPath+"conc_desc_en-UK.yaml", ")")
			return nil, false
		}
		err = yaml.Unmarshal(yamlFile, d)
		if err != nil {
...
func (c *ConcConfig)ReadConf() (*ConcConfig, bool) {
	...
	if concPath == "" { concPath = ".\\" }
	confPath := concPath+"conf"+string(os.PathSeparator)
	yamlFile, err := ioutil.ReadFile(confPath+"conc_conf.yaml")
	if err != nil {
		HdrErr()
		fmt.Println("Unable to reach config document (in", confPath+"conc_conf.yaml", ")")
		return nil, false
	}
