...
func RunCli() {
	...
	var isClientZipNeeded = false
	var isClientEncryptNeeded = false
	...
	app.Commands = []cli.Command {
		...
		// client command
		{
			...
			Flags: []cli.Flag {
				...
				cli.BoolFlag {
					Name:        "zip-needed, z",
					Usage:       "enable compress before sending a folder",
					Destination: &isClientZipNeeded,
				},
				// 需要加密
				cli.BoolFlag {
					Name:        "encrypt-needed, en",
					Usage:       "enable encrypt transferring",
					Destination: &isClientEncryptNeeded,
				},
			},

			// 定义Client的动作
			Action: func(c *cli.Context) error {
				ret := client.NewMonitor(true, clientMonitorPort, clientErrInfoPort, isClientZipNeeded, isClientEncryptNeeded)
				pref.HdrInf()
