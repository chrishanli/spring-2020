...
import (
	...
	"errors"
	"fmt"
	"github.com/schollz/progressbar/v3" // 进度条插件
	"io"
	"net"
	"os"
	"strings"
...
// 接收或重写文件
func RecvFile(seek int64, filePath string, fileNum int32, fileSize int64, chksum *[]byte, conn net.Conn, key []byte, bufferSize int64) (bool, error) {
	// 不停接收文件，直到接收到正确的文件为止
	...
	for {
		...
		if err != nil {
			...
			if err != nil {
				pref.HdrWrn()
				fmt.Println("Connection was broken.")
				return false, err
			}
		}
		...
		// 7/5/2020按照是否要解密来接收每一封文件
		if key != nil {
			buf = make([]byte, bufferSize*4 + int64(len(key)))
			for receivedSize < fileSize {
				...
				// 解密
				decryptedBuff := cipher.AESDecrypt(buf[:n], key)
				// 写入文件
				f.Write(decryptedBuff)
				receivedSize += int64(len(decryptedBuff))
				bar.Add(len(decryptedBuff))
			}
		} else {
...
func Server(working_d string, addr string, port string, buffer_size int64) {
	...
	needUnZip := false
	needDecrypt := false
	var decryptKey []byte = nil

	for { // connect
		needUnZip = false		// 设置缺省操作
		needDecrypt = false
		pref.HdrInf()
		...
		for { // STR received.
			...
			if cmd == "FIL" {          // 收到FIL指令
				...
				if userNeedsOverwrite {
					var succ bool
					if needDecrypt {
						succ, err = RecvFile(0, currFileAbsPath, fileNum, fileSize, &chksum, conn, decryptKey, buffer_size)
					} else {
						succ, err = RecvFile(0, currFileAbsPath, fileNum, fileSize, &chksum, conn, nil, buffer_size)
					}
					// 检视接收结果
					if succ != true || err != nil {
						...
						if io.EOF == err || syscall.EPIPE == err {
							...
							pref.HdrWrn()
							fmt.Println("Connection is now closed.")
							errstr := err.Error()
					...
					// 如果需要解压，就尝试解压文件夹
					if needUnZip {
						err = utils.Unzip(currFileAbsPath, rootDir)
						if err != nil {
							pref.HdrErr()
							fmt.Println("Unzip reported an error.")
						}
						err = os.Remove(currFileAbsPath)
					}
					...
					pref.HdrInf()
					fmt.Println("The file: ", currFileRelPath, "was skipped.")
				}
			} else if cmd == "CHD" {
				// 获取相对于根目录的相对目录
				for n = 6; buf[n] != '>'; n++ {
				...
				// 如果存在文件，就返回当前的size, 如果不存在文件，就返回size为0，诱导客户端重传
				if exists {
					// 制作EPT指令
					ept := make([]byte, 15)
				...
				var succ bool
				if needDecrypt {
					succ, err = RecvFile(seek, currFileAbsPath, fileNum, fileSize, &chksum, conn, decryptKey, buffer_size)
				} else {
					succ, err = RecvFile(seek, currFileAbsPath, fileNum, fileSize, &chksum, conn, nil, buffer_size)
				}
				// 检视接收结果
				if succ != true || err != nil {
					// 文件有问题	或 提前遇到终止
					if io.EOF == err {
						...
						pref.HdrWrn()
						fmt.Println("Connection is no longer in use.")
						errstr := err.Error()
						_, err := conn.Write([]byte("ERR <" + errstr[0:3] + "><" + errstr[4:]))
						if err != nil {
							isConnected = false
				...
				needUnZip = true
				pref.HdrInf()
				fmt.Println("[ZIP] User needs to zip.")
				_, err = conn.Write([]byte("YES <ZIP>"))
			} else if cmd == "ENC" {  // 用户需要加密及解密
				// 生成RSA公钥私钥
				pubKey, priKey, err := cipher.GenRSAKeys(1024)
				// 发送RSA公钥，如果没有出错
				if err != nil {
					pref.HdrErr()
					fmt.Println("RSAKeys was unable to generate.")
					_, err = conn.Write([]byte("ERR <>"))
					continue
				}
				copy(buf[0:], "PUB <")
				copy(buf[5:], utils.Int32ToBytes(int32(len(pubKey))))
				copy(buf[9:], "><")
				copy(buf[11:], pubKey)
				_, err = conn.Write(buf)
				// 接收密码（密文）
				_, err = conn.Read(buf)
				decryptKeyCiphered := buf[11:11 + utils.BytesToInt32(buf[5:9])]
				// 解开密码
				decryptKey = cipher.RSADecrypt(decryptKeyCiphered, priKey)
				if decryptKey == nil {
					pref.HdrErr()
					fmt.Println("Dectypt key was unable to receive: ciphered key:", decryptKeyCiphered)
					_, err = conn.Write([]byte("ERR <>"))
					continue
				}
				// 密码已经收到
				_, err = conn.Write([]byte("YES <KEY>"))
				needDecrypt = true
				pref.HdrInf()
				fmt.Println("[ENC] User needs to transfer in encrypted format.")
			}
